...
import (
	...
	"github.com/hiagomf/bank-api/server/infraestructure/records/account_owner_address/postgres"
	"github.com/hiagomf/bank-api/server/oops"
	"github.com/hiagomf/bank-api/server/utils"
)

type repository struct {
	pg *postgres.PGAccountOwnerAddress
}

func novoRepo(db *database.DBTransaction) *repository {
	return &repository{
		pg: &postgres.PGAccountOwnerAddress{DB: db},
	}
}

// Insert - insere o titular da conta
func (r *repository) Insert(data *account_owner_address.AccountOwnerAddress) (err error) {
	return r.pg.Insert(data)
}

// Update - altera os dados do titular da conta
func (r *repository) Update(data *account_owner_address.AccountOwnerAddress) (err error) {
	return r.pg.Update(data)
}

// SelectOne - busca o titular da conta
func (r *repository) SelectOne(id *int64) (res *account_owner_address.AccountOwnerAddress, err error) {
	return r.pg.SelectOne(id)
}

// SelectPaginated - lista titulares das contas
func (r *repository) SelectPaginated(parameters *utils.ParametrosRequisicao) (res *account_owner_address.AccountOwnerAddressPag, err error) {
...
func (r *repository) DisableAllActives(ownerID *int64) (err error) {
	return r.pg.DisableAllActives(ownerID)
}

// ConvertToInfra - realiza a conversão de alguma model informada para model de infraestructure
func (r *repository) ConvertToInfra(data interface{}) (res *account_owner_address.AccountOwnerAddress, err error) {
	res = &account_owner_address.AccountOwnerAddress{}
